...
class MusicBot(discord.Client):
    ...
    async def _join_startup_channels(self, channels, *, autoplay=True):
        joined_servers = set()
            ...
            if server.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        ...
                        'Use %splay to play it to your voice channel.' % self.config.command_prefix)
...
#######################################################################################################################
    ...
    async def on_ready(self):
        ...
        if self.config.show_config_at_start:
            ...
            log.info("  Auto-Play: " + ['Disabled', 'Enabled'][self.config.auto_play])
        ...
        await self._join_startup_channels(autojoin_channels, autoplay=self.config.auto_play)
    ...
    async def cmd_join(self, channel, server, author, voice_channel):
        ...
        return Response(self.str.get('cmd-play-reply', 'Connected to `{0.name}`').format(author.voice_channel))
